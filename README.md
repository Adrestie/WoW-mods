...
| [mod-spheregrid](mod-spheregrid/) | A second progression on a grid of cells the character walks through, bought with a currency the content awards: stat stones, runes that add ranks to the game's own spells, and cells teaching custom class spells with backported visuals. An in-game grid editor and a player interface over AIO. Ships its own client patch and writes into a client that already has one, moving its identifiers aside when a server has taken them. Original module. |
