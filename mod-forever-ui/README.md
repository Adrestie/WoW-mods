...
| Character sheet | The window and its two panes, the 3D model, the equipment slots, the side tabs, the level and class line, resistances, the twelve stat lines and their two category selectors; and in the right pane the two tabs it opens on -- character stats, and the equipment manager: set cards in the player's own order, the worn-set check, rename and delete, `New Set`, and the icon picker beside the sheet. The right pane folds away, and the side tabs stay where they are. | the character tab is done; the side tabs it stands beside -- pet, reputation, skills, PvP, currency, statistics -- open their screen but are not laid out yet |
