...
--                             (37, -66) ou (19, -42)
--   WorldMap_ToggleSizeUp     plein ecran -- l'etape 4 ; ici on se retire
...
local function construireFond(carte)
	...
	-- OverscrollBG, sur le rectangle de la carte. Ses textures restent des
	-- REGIONS du fond, et non un cadre fils : un cadre fils prend un niveau
	-- au-dessus de son parent -- 89 --, soit AU-DESSUS des tuiles de WotLK
	-- (WorldMapDetailFrame, 88), qu'il masquait en partie. La tuile est un
	-- atlas : 3.3.5 ne pave pas un rectangle d'atlas, on l'etire.
	local x1, y1 = G.carteX, G.carteY
	local x2, y2 = G.carteX + G.carteL, G.carteY - G.carteH
	local tuile = fond:CreateTexture(nil, "BACKGROUND")
	ForeverUI.SetAtlas(tuile, "gamepad-mapquestlog-bgtile-2k", true)
	tuile:SetPoint("TOPLEFT", fond, "TOPLEFT", x1, y1)
	tuile:SetPoint("BOTTOMRIGHT", fond, "TOPLEFT", x2, y2)
	fond.tuile = tuile
	...
	if e then
		...
		local quarts = {
			{ x1, y1, cx, cy, e[2], e[3], e[4], e[5] },
			{ cx, y1, x2, cy, e[3], e[2], e[4], e[5] },
			{ x1, cy, cx, y2, e[2], e[3], e[5], e[4] },
			{ cx, cy, x2, y2, e[3], e[2], e[5], e[4] },
		}
		for _, q in ipairs(quarts) do
			local v = fond:CreateTexture(nil, "BORDER")
			v:SetTexture(e[1])
			v:SetTexCoord(q[5], q[6], q[7], q[8])
			v:SetPoint("TOPLEFT", fond, "TOPLEFT", q[1], q[2])
			v:SetPoint("BOTTOMRIGHT", fond, "TOPLEFT", q[3], q[4])
		end
	end
	return fond
end
...
local function ouvrirMenu(ancre, liste)
...
local function construireEtages(carte)
	...
	ForeverUI.SetAtlas(fond, "common-dropdown-textholder", true)
...
local function rafraichirEtages()
	...
	if enPetiteFenetre() and GetNumDungeonMapLevels and GetNumDungeonMapLevels() > 0 then
		b.Text:SetText(nomEtage(GetCurrentMapDungeonLevel()))
		b:Show()
	else
		b:Hide()
	end
...
local function poserReduit()
	...
	W.fond:SetFrameLevel(carte:GetFrameLevel())
	...
	titre:SetParent(W.cadre.bandeau)
	titre:ClearAllPoints()
	titre:SetPoint("TOP", W.cadre.bandeau, "TOP", 0, G.titreTexteY)
	titre:SetPoint("LEFT", W.cadre.bandeau, "LEFT")
	titre:SetPoint("RIGHT", W.cadre.bandeau, "RIGHT")
	...
	WorldMapFrameCloseButton:SetPoint("TOPRIGHT", carte, "TOPRIGHT", G.fermerX, G.fermerY)
	WorldMapFrameSizeUpButton:ClearAllPoints()
	WorldMapFrameSizeUpButton:SetPoint("RIGHT", WorldMapFrameCloseButton, "LEFT", G.agrandirX, 0)
...
-- LE PLEIN ECRAN est celui de WotLK jusqu'a l'etape 4 : on s'efface, et on
-- rend au client ce qu'on lui avait pris.
...
if hooksecurefunc then
	...
	if WorldMap_ToggleSizeUp then
		hooksecurefunc("WorldMap_ToggleSizeUp", retirer)
	end
	...
	if WorldMapFrame_SetMapName then
		hooksecurefunc("WorldMapFrame_SetMapName", function()
			if W.construit and enPetiteFenetre() then
				WorldMapFrameTitle:SetText(TEXTE.titre)
			end
		end)
	end
	...
	if WorldMapFrame_DisplayQuests then
		hooksecurefunc("WorldMapFrame_DisplayQuests", function()
			if W.construit and enPetiteFenetre() and WorldMapTrackQuest then
				WorldMapTrackQuest:SetAlpha(0)
				WorldMapTrackQuest:EnableMouse(false)
			end
		end)
