...
| **Deux icônes d'onglet sont introuvables, et le texte reste en attendant** | `CHARACTER_MODE_TAB_ICONS` nomme `INV_SideTab_Reputation2_c60`, `Ability_Racial_JackofAllTrades` et `INV_SideTab_Currency_c60`. Seule la deuxième a pu être versée. Le client chargé dans wow.export **est** celui de camelot — `wow_classic_beta 1.60.1.69913` — et son code nomme les trois, mais le listfile communautaire ne donne aucun nom aux deux `INV_SideTab_*_c60` : `/resolve` rend `null`, `/search` n'en connaît aucune, et rien ne permet donc de les exporter par leur chemin. Un listfile à jour est le seul déblocage. Réputation et monnaie gardent le texte du client d'ici là, plutôt qu'une icône inventée. |
