...
| Portrait du sac | 36 x 36 en (−4, 1), sa taille et sa place d'origine, dans le calque **`BORDER`** | `SetPortraitTextureSizeAndOffset(36, −4, 1)` est respecté. Le calque décide, pas l'ordre de création : le fond est en `BACKGROUND`, le métal en `OVERLAY`, `BORDER` est libre entre les deux. Auparavant le portrait était dans le calque du fond, et le corps de celui-ci — ancré à 20 px du haut — passait devant lui et coupait l'anneau en deux. **RESTE À FAIRE : le masquage.** camelot arrondit avec `PortraitContainer.CircleMask` ; ce client ne sait pas masquer une texture (voir §2), donc les coins de l'icône dépassent de l'anneau. Réduire l'icône pour qu'elle tienne dans le trou n'est pas masquer : essayé à 28, refusé |
