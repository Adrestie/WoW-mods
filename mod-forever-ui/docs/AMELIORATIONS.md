...
| **La jauge circulaire n'est pas portable** | camelot la fait avec un `Cooldown` dont il remplace la texture de balayage — `SetSwipeTexture`, **absent** du binaire de 3.3.5, vérifié. L'anneau, la lueur, le fond de faction et le badge sont posés ; la progression s'écrit en toutes lettres, comme le fait déjà `CurrentRankProgressField`. `GameFontNormalMed2` n'existe pas non plus, et le compte à rebours de fin de saison demande un `C_SeasonInfo` absent. |
