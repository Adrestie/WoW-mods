...
| Une feuille de **2048 de large** sort en bruit | l'anneau d'autolancement du familier venait de `uiactionbarfx` (2048 × 1024) et s'affichait illisible. Le fichier est bon : relu avec `tools/foreverui/blp.py`, il est identique à son aperçu. Toutes les feuilles que l'addon affiche correctement font **1024 au plus** | `tools/petite_feuille.py` redécoupe les éléments voulus dans une petite feuille en puissance de deux, écrit le `.blp` (BGRA non compressé, la forme des autres feuilles) avec son aperçu, et génère `UIAtlas_07_petites_feuilles.lua` qui **redéfinit** ces noms — la table est plate, la dernière définition gagne |
